...
import { DashboardStatsModern } from "./components/DashboardStatsModern";
import { ControlsBarModern, StockFilterOption, SortOption, ViewMode } from "./components/ControlsBarModern";
import { ProductTableModern } from "./components/ProductTableModern";
import { ProductGridModern } from "./components/ProductGridModern";
import { ProductModalModern } from "./components/modal/ProductModalModern";
import { DeleteModalModern } from "./components/modal/DeleteModalModern";
import { ToastModernContainer, ToastMessage } from "./components/ui/ToastModern";
import {
  addProductAPI,
  deleteAPI,
  editProductAPI,
...
export const InventoryWorkspaceModern: React.FC = () => {
  ...
  // Search input handler with API sync
  ...
  // GSAP View Switching Smooth Transition
  useGSAP(
    () => {
      const prefersReduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
      if (prefersReduced || !viewContainerRef.current) return;
  ...
  return (
    <div ref={mainContainerRef} className="min-h-screen bg-slate-50 text-slate-900 font-sans antialiased pb-12">
      {/* Header */}
      <NavbarModern
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
        {/* Statistics Strip */}
        <DashboardStatsModern products={products} />
        ...
        {/* Dynamic Content View with GSAP transition */}
        <div ref={viewContainerRef}>
          {loading ? (
            <div className="bg-white border border-slate-200 rounded-md p-12 text-center my-2 shadow-2xs">
            ...
            <div className="bg-white border border-slate-200 rounded-md p-10 text-center my-2 shadow-2xs">
      ...
      {/* Add / Edit Product Modal with GSAP Motion */}
      <ProductModalModern
      ...
      {/* Delete Confirmation Modal with GSAP Motion */}
      <DeleteModalModern
      ...
      <ToastModernContainer toasts={toasts} onDismiss={dismissToast} />
    </div>
  );
};

export default InventoryWorkspaceModern;
